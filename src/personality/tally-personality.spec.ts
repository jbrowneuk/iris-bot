...
import { addCommand, collection, resetCountCommand, TallyPersonality, totalCountCommand } from './tally-personality';
...
describe('Tally functionality', () => {
  ...
  describe('Adding to the tally behaviour', () => {
    ...
    it('should create a record for the guild if it doesn’t exist', done => {
      mockDatabase.setup(m => m.getRecordsFromCollection(It.isAny(), It.isAny())).returns(() => Promise.resolve([]));

      mockDatabase.setup(m => m.insertRecordsToCollection(It.isValue(collection), It.isAny())).returns(() => Promise.resolve());

      personality.onMessage(mockMessage.object).then(() => {
        mockDatabase.verify(m => m.insertRecordsToCollection(It.isValue(collection), It.isObjectWith({ guildId: mockGuildId })), Times.once());
        done();
      });
    });
    ...
    it('should update the record for the guild if it exists', done => {
      ...
      mockDatabase.setup(m => m.getRecordsFromCollection(It.isAny(), It.isAny())).returns(() => Promise.resolve([{ count: initialCount }]));

      mockDatabase.setup(m => m.updateRecordsInCollection(It.isValue(collection), It.isAny(), It.isAny())).returns(() => Promise.resolve());

      personality.onMessage(mockMessage.object).then(() => {
        mockDatabase.verify(
          m => m.updateRecordsInCollection(It.isValue(collection), It.isObjectWith({ $count: initialCount + 1 }), It.isObjectWith({ $guildId: mockGuildId })),
          Times.once()
        );
        done();
      });
  ...
  describe('Reset tally behaviour', () => {
    ...
    it('should do nothing if the guild record doesn’t exist', done => {
      ...
      personality.onMessage(mockMessage.object).then(() => {
        mockDatabase.verify(m => m.updateRecordsInCollection(It.isValue(collection), It.isAny(), It.isAny()), Times.never());
        done();
      });
    });

    it('should update the record for the guild if it exists', done => {
      mockDatabase.setup(m => m.getRecordsFromCollection(It.isAny(), It.isAny())).returns(() => Promise.resolve([{ count: 64 }]));

      mockDatabase.setup(m => m.updateRecordsInCollection(It.isValue(collection), It.isAny(), It.isAny())).returns(() => Promise.resolve());

      personality.onMessage(mockMessage.object).then(() => {
        mockDatabase.verify(
          m => m.updateRecordsInCollection(It.isValue(collection), It.isObjectWith({ $count: 0 }), It.isObjectWith({ $guildId: mockGuildId })),
          Times.once()
        );
        done();
      });
  ...
  describe('Get tally behaviour', () => {
    ...
    it('should resolve to string containing count if the guild record doesn’t exist', done => {
      const count = 64;
      mockDatabase.setup(m => m.getRecordsFromCollection(It.isAny(), It.isAny())).returns(() => Promise.resolve([{ count }]));
